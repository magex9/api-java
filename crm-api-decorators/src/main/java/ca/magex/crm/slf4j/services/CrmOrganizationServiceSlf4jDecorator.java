...
public class CrmOrganizationServiceSlf4jDecorator implements CrmOrganizationService {
	...
	public OrganizationDetails findOrganizationByDisplayName(String displayName) {
		if (logger.isTraceEnabled()) {
			long start = System.nanoTime();
			try {
				logger.trace("Calling findOrganizationByDisplayName(" + displayName + ")");
				OrganizationDetails result = delegate.findOrganizationByDisplayName(displayName);
				logger.trace("Executed findOrganizationByDisplayName(" + displayName + ") in " + Duration.ofNanos(System.nanoTime() - start) + " (" + result.getClass() + ": " + result + ").");
				return result;
			}
			catch (Exception e) {
				logger.trace("Exception on findOrganizationByDisplayName(" + displayName + ") in " + Duration.ofNanos(System.nanoTime() - start) + " (" + e.getClass() + ": " + e.getMessage() + ").");
				throw e;
			}
		}
		else if (logger.isDebugEnabled()) {
			long start = System.nanoTime();
			try {
				logger.debug("Calling findOrganizationByDisplayName(" + displayName + ")");
				OrganizationDetails result = delegate.findOrganizationByDisplayName(displayName);
				logger.debug("Executed findOrganizationByDisplayName(" + displayName + ") in " + Duration.ofNanos(System.nanoTime() - start) + ".");
				return result;
			}
			catch (Exception e) {
				logger.debug("Exception on findOrganizationByDisplayName(" + displayName + ") in " + Duration.ofNanos(System.nanoTime() - start) + " (" + e.getClass() + ": " + e.getMessage() + ").");
				throw e;
			}
		}
		else if (logger.isInfoEnabled()) {
			logger.info("Calling findOrganizationByDisplayName(" + displayName + ")");
			return delegate.findOrganizationByDisplayName(displayName);
		}
		else {
			return delegate.findOrganizationByDisplayName(displayName);
		}
